...
class CreateRouteTables extends Migration
{
    ...
    public function up()
    {
        Schema::create('routes', function (Blueprint $table) {
            ...
            $table->char('uuid', 32)->default('')->comment('入口的uuid,和权限的permission_uuid 值一样,一个入口就是一个访问权限，uuid一样');
            $table->string('guard_name');
            $table->string('route', 255)->default('')->comment('入口的路由链接，前端的路由');
            $table->string('name', 255)->default('')->comment('入口的标识，唯一')->unique();
        ...
        Schema::create('route_menus', function (Blueprint $table) {
            ...
            $table->char('parent_uuid', 32)->default('')->index();
            $table->string('guard_name');
            $table->string('name', 255)->default('')->comment('访问入口分类标识名称，唯一')->unique();
        ...
        Schema::create('route_to_menus', function (Blueprint $table) {
            ...
            $table->char('route_uuid', 32)->default('');
            $table->char('route_menu_uuid', 32)->default('');
